...
namespace three {
	...
	protected:
		...
		void buildPlane(Side u, Side v,
			...
			auto offset = (float)vertices.size();
			...
			for (int iy = 0; iy < gridY; iy++) {
				for (int ix = 0; ix < gridX; ix++) {
					...
					auto a = ix + gridX1 * iy;
					auto b = ix + gridX1 * (iy + 1.f);
					auto c = (ix + 1.f) + gridX1 * (iy + 1.f);
					auto d = (ix + 1.f) + gridX1 * iy;

					auto uva = Vector2(ix / gridX, 1.f - iy / gridY);
					auto uvb = Vector2(ix / gridX, 1.f - (iy + 1.f) / gridY);
					auto uvc = Vector2((ix + 1.f) / gridX, 1.f - (iy + 1) / gridY);
					auto uvd = Vector2((ix + 1.f) / gridX, 1.f - iy / gridY);
