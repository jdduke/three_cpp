    ...
    //if ( material.attributes.size() > 0 )
    {
        ...
        for (auto& namedAttribute : material.attributes)
        {
            ...
            if (!attribute.__glInitialized || attribute.createUniqueBuffers)
            {
                ...
                if (attribute.type == THREE::v2)
                    size = 2;
                else if (attribute.type == THREE::v3)
                    size = 3;
                else if (attribute.type == THREE::v4)
                    size = 4;
                else if (attribute.type == THREE::c)
                    size = 3;
...
void GLRenderer::setParticleBuffers(Geometry& geometry, int hint, Object3D& object)
{
    ...
    else
    {
        ...
        for (int i = 0, il = (int)customAttributes.size(); i < il; i++)
        {
            ...
            if (customAttribute.needsUpdate && (customAttribute.boundTo.empty() || customAttribute.boundTo == "vertices"))
            {
...
void GLRenderer::renderBuffer(Camera& camera, Lights& lights, IFog* fog, Material& material, GeometryGroup& geometryGroup, Object3D& object)
{
    ...
    if (updateBuffers)
    {
        ...
        if (material.skinning && attributes[AttributeKey::skinVertexA()].valid() && attributes[AttributeKey::skinVertexB()].valid() && attributes[AttributeKey::skinIndex()].valid() && attributes[AttributeKey::skinWeight()].valid())
        {
...
void GLRenderer::addObject(Object3D& object, Scene& scene)
{
    ...
    if (!object.glData.__glActive)
    {
        ...
        else if (object.type() == THREE::Ribbon || object.type() == THREE::Line || object.type() == THREE::ParticleSystem)
        {
...
void GLRenderer::removeObject(Object3D& object, Scene& scene)
{
    ...
    if (object.type() == THREE::Mesh || object.type() == THREE::ParticleSystem || object.type() == THREE::Ribbon || object.type() == THREE::Line)
    {
...
Program& GLRenderer::setProgram(Camera& camera, Lights& lights, IFog* fog, Material& material, Object3D& object)
{
    ...
    if (refreshMaterial)
    {
        ...
        if (material.type() == THREE::MeshPhongMaterial || material.type() == THREE::MeshLambertMaterial || material.lights)
        {
        ...
        if (material.type() == THREE::ShaderMaterial || material.type() == THREE::MeshPhongMaterial || material.envMap)
        {
        ...
        if (material.type() == THREE::MeshPhongMaterial || material.type() == THREE::MeshLambertMaterial || material.type() == THREE::ShaderMaterial || material.skinning)
        {
...
void GLRenderer::loadUniformsGeneric(Program& program, UniformsList& uniforms, bool warnIfNotFound)
{
    ...
    for (const auto& uniformAndKey : uniforms)
    { //( size_t j = 0, jl = uniforms.size(); j < jl; j ++ ) {
...
void GLRenderer::setupLights(Program& program, Lights& lights)
{
    ...
    for (size_t l = dlength * 3, ll = dcolors.size(); l < ll; l++)
        dcolors[l] = 0.0;
    for (size_t l = plength * 3, ll = pcolors.size(); l < ll; l++)
        pcolors[l] = 0.0;
    for (size_t l = slength * 3, ll = scolors.size(); l < ll; l++)
        scolors[l] = 0.0;
    for (size_t l = hlength * 3, ll = hskyColors.size(); l < ll; l++)
        hskyColors[l] = 0.0;
    for (size_t l = hlength * 3, ll = hgroundColors.size(); l < ll; l++)
        hgroundColors[l] = 0.0;
        ...
        if (object.type() == THREE::SkinnedMesh)
        {
            ...
            if (maxBones < (int)object.bones.size())
            {
                console().warn() << "WebGLRenderer: too many bones - "
                                 << object.bones.size() << ", this GPU supports just " << maxBones << " (try OpenGL instead of ANGLE)";
            }
        }
